...
public class TypeController {
    ...
    public String List(@PageableDefault(size = 6,sort = {"id"},direction = Sort.Direction.DESC) Pageable pageable, Model model){
        ...
        int pageNum = types.getTotalPages();
        long totalElements = types.getTotalElements();
        model.addAttribute("typesSize",totalElements);
    ...
    public String addTypes(@Valid Type type,BindingResult result, RedirectAttributes attributes){
        Type findByName = typeService.getTypeByName(type.getName());
        if (findByName != null) {
    ...
    public String detelePost(@PathVariable Long id, RedirectAttributes attributes) {
